            ...
            <div>
              ...
              <div className="flex gap-2 mt-1 items-start">
                <div className="size-16 rounded-lg overflow-hidden bg-muted border border-border shrink-0">
                  {p.image_urls[0] && <SafeImage src={p.image_urls[0]} alt={p.name || (p.kind === "tool" ? "Tool image" : "Product image")} className="w-full h-full object-cover" />}
                </div>
                    ...
                    {upliftQuote.paymentRequired ? (
                      <div className="space-y-2 text-[12px] font-body text-foreground/85 leading-snug">
                        ...
                        <div className="flex items-baseline justify-between gap-2 border-t border-border/60 pt-2 font-medium">
                          <span>Total to pay if approved</span>
                          <span className="tabular-nums">{money(upliftQuote.totalPence)}</span>
                        </div>
                        <p className="text-[11px] text-muted-foreground">
                          Nothing is charged now. An admin reviews the change first — if it's approved you'll be asked to pay{" "}
                          {money(upliftQuote.totalPence)} to activate the new audience. Your campaign keeps running on its current audience
                          and rate until then.
                        </p>
                ...
                return (
                  ...
                  >
                    ...
                    <span className="flex-1 min-w-0">
                      <span className="block font-body text-[13px] leading-snug [overflow-wrap:anywhere] break-words">
                        {item.name}
                      </span>
