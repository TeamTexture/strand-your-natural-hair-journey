...
/**
 * Brand forgot-password entry. Posts to the shared password-reset edge function
 * with audience "brand", which mints a single-use native recovery link and
 * delivers it via Resend from noreply@mystrand.co.uk.
 */
const BrandForgotPassword = () => {
    ...
    try {
      const { error: fnError } = await supabase.functions.invoke("pro-password-reset", {
        body: {
          email: value,
          audience: "brand",
          redirectTo: `${window.location.origin}/brand/reset-password`,
        },
      });
      if (fnError) {
        ...
        <div className="flex flex-col items-center text-center mb-6">
          ...
          <p className="font-body text-[13px] text-foreground/70 max-w-[280px] mt-2 leading-snug">
            {sent
              ...
              : "Enter the email on your brand account and we'll send you a secure link to set a new password."}
          </p>
        </div>

        {sent ? (
          <div className="space-y-3">
            <Button variant="gold" size="pill" onClick={() => nav("/brand/auth?mode=signin")}>
              Back to sign in →
            </Button>
            <button
              type="button"
          ...
          <form onSubmit={submit} className="space-y-4 selectable">
            ...
            <button
              type="button"
              onClick={() => nav("/brand/auth?mode=signin")}
              className="w-full text-center text-xs text-muted-foreground hover:text-foreground"
            >
              Back to sign in
            </button>
