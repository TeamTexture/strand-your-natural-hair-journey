...
### Step 3 — `product-analyse` *(dual-photo vision + web research, "wow moment")*
- **Model:** `claude-sonnet-4-6` (vision-capable, native web_search support).
- **Input shape (revised 2026-04-27):** `{ photos: { front: <bytes-or-data-url>, back: <bytes-or-data-url> } }`. **Both required** when `STRAND_AI_PROVIDER_PRODUCT_PHOTO=claude`. The Lovable+Gemini path retains single-photo input for back-compat (`{ photo: <bytes> }`) until that flag is flipped — no behaviour change for existing Gemini callers. Server-side validation: if `STRAND_AI_PROVIDER_PRODUCT_PHOTO=claude` and either photo is missing, reject with HTTP 400 and the message `"STRAND needs both the front and back of the product to give you a full analysis."` No silent degradation to single-photo, no escape hatch — the dual-photo contract is strict.
- **Tools provided to Claude:**
  1. **Two vision content blocks** in the user message — Anthropic shape: `{ type: "image", source: { type: "base64", media_type, data } }` for data URLs; `{ type: "image", source: { type: "url", url } }` for signed URLs. Order matters: front first, back second. Each is followed (or preceded) by a text block labelling which photo it is so Claude can attribute facts correctly: front carries brand + product name + marketing claims; back carries ingredient panel + usage instructions + regulatory text.
  2. **`web_search` tool** — Anthropic's native web search tool definition with `max_uses: 4` (one to confirm brand+product identity when the front is partial or stylised, optional cross-check for full INCI if the back panel is folded / cut off / glare-obscured, optional brand/formulation context, optional retailer fact). Tight upper bound; Claude decides per call whether to invoke.
...
- **Autonomous reasoning loop:** single API call. Claude reads brand and product name primarily from the front photo, reads the ingredient panel and directions primarily from the back photo, decides whether to search the web (typically when one side is partial — folded back-of-bottle, stylised front-of-bottle obscuring the brand, indie product not on Google's first page), runs the search itself, then composes the analysis. No client-side orchestration; Anthropic returns the final `tool_use` once Claude has enough context.
 - **Task instructions update (revised 2026-04-27):** *"You're looking at two photos of the same product — front (brand + product name) and back (ingredient panel + usage instructions). Read both. If either is missing, blurry, or in the wrong language, use web_search to fill the gap. The full canonical product details should be reflected in your `return_product_analysis` output: `ingredients[]` is the COMPLETE INCI list, `product_name` and `brand` match what the brand actually calls it, `category` and `use_cases` align with the brand's positioning."*
   - **(revised 2026-04-29) tightness caps: use_cases ≤2, tips ≤2, ai_summary ≤3 sentences, key_ingredients ≤6, signal selection by relevance.** Output was running long and surfacing every available user signal on every product. New prompt rules cap the arrays and instruct Claude to pick the 1–2 personal signals that a clinical hair coach would prioritise for THIS product (e.g. scalp condition + dermatologist context for an exfoliator; porosity + hard water for a deep conditioner; ferritin + diagnosed conditions for a hair-loss treatment). Schema is unchanged — caps are prompt-only so we can keep tuning without a migration.
...
- **Token cost (revised 2026-04-27):** front image ~1500 tokens + back image ~1500 tokens + ~2.2K text input + ~800 output base + ~2–3 web_search invocations at Anthropic's per-search price (~$0.01/search, late-2025 pricing) + additional output tokens from synthesised research → **~$0.04–0.06 per call** (was ~$0.03–0.05 single-photo+search; was ~$0.014 vision-only). Vision+research with two images is now the most expensive Phase 2 per-call op; budget accordingly in §6 cost projections.
...
- **Schema unchanged.** The `return_product_analysis` tool definition is identical to the vision-only spec — what changes is how Claude *fills* the schema (single-image vision-only → dual-image vision+research). Existing client-side rendering in `useProductScan.ts` and `ProductDetailNew.tsx` continues to work without modification on the *render* side; the *capture* side needs the guided two-step flow described under "UX rationale" below.
- **Verification:** match outputs against the four sample reports at the repo root before flipping the call site (`STRAND-ingredient-report-Maya.pdf`, etc.). **Plus** the new dual-photo partial-label smoke test in §8 Step 11b (front + back of a product where each side carries different gaps — brand-only front, partial-INCI back — must produce a full INCI list via web search and the brand's actual product name, not just descriptors).
- **Rollout flag:** `STRAND_AI_PROVIDER_PRODUCT_PHOTO=claude`.

#### UX rationale (added 2026-04-27, Paige's request)

Why dual-photo, not single-photo + web search:

- **Front-only with web search** still fails on indie brands where the product isn't on Google's first page. Without the ingredient panel, Claude has nothing local to fall back on — and indie / direct-to-consumer Black-owned brands (a meaningful share of the STRAND user's shelf) are exactly the long tail Google doesn't index well.
- **Back-only (ingredient-panel-only)** loses brand context. Two products from different brands can share most of their INCI; the back panel alone can't pin which one the user is holding, so `product_name` ends up generic ("Curl cream") rather than the brand's actual SKU name.
- **Both is the answer.** Front pins identity (brand + product name + marketing claims); back pins composition (full INCI + directions). Together they give Claude two strong signals that confirm each other, with `web_search` as the third source only when one side is genuinely insufficient. UX modelled on Yuka's food-label scanning, which uses the same dual-capture pattern for the same reason.

Client UX (guided two-step flow):

- Step 1 of capture: client opens the camera with the prompt **"Photograph the front of the product"**. User captures front photo.
- Step 2 auto-advances: prompt becomes **"Now photograph the back where the ingredients are listed"**. User captures back photo.
- Both photos are required to submit; the submit button is disabled until both are present.
- **No "I only have one" escape** — single-photo input is unsupported when `STRAND_AI_PROVIDER_PRODUCT_PHOTO=claude`. If the user somehow submits with one photo missing (network glitch, race in the capture state), the server returns the strict 400 above and the client surfaces it as a retry prompt.
- **Scope note:** these client UX changes are tracked in this audit for spec completeness but are **NOT part of the Step 3 server PR**. The server PR ships the strict dual-photo validation and the dual-image Anthropic call. The client guided-capture flow ships in a parallel small PR (call site updates in `ProductScanning.tsx` and `useProductScan.ts`) so the server and client land independently and can be reviewed separately. Sequence: server PR ships first (rejects single-photo), then client PR ships the guided-capture UX. Between the two PRs, the existing single-photo client path will be broken under the `claude` flag — keep `STRAND_AI_PROVIDER_PRODUCT_PHOTO=lovable` until both PRs are live.
...
**Step 11b. [Paige]** — *(Added 2026-04-26 with the vision+web_search revision; revised 2026-04-27 to dual-photo capture.)* Dual-photo partial-label web-search smoke. Photograph **the FRONT and BACK** of a product where the front carries partial label info (brand visible, product name partly hidden behind stylised typography) AND the back carries a partial INCI (panel folded / cut off / glare-obscured) — both sides have gaps that Claude has to resolve.
*Verify:* `ingredients[]` returns the **FULL** INCI list (resolved via `web_search` cross-checking what each photo couldn't fully capture, not just OCR of the visible fragments); `product_name` is the brand's actual SKU name (not just a descriptor like "Curl cream"); `brand`, `category`, and `use_cases` populate as they would if the user had pasted a URL. `ai_summary` and `tips` may reference brand/formulation facts inline; the formal "Read more — How To Love Your Afro, Chapter X: …, p.[page]" footer still appears whenever guidance is rooted in the book; web-derived facts do NOT appear under the "Read more" line. Also confirm the strict-validation path: submit with only the front (or only the back) and verify the server returns HTTP 400 with the message *"STRAND needs both the front and back of the product to give you a full analysis."* — no silent fallback to single-photo. **No Publish click — server-only PR.** The client guided-capture UX ships in a separate small PR after the server PR is verified green.
...
### 2026-04-27 — Step 3 input changes from single photo to dual photo (front + back)

UX modelled on Yuka's food-label scanning. Single-photo + web search still fails on indie brands not on Google's first page; ingredient-only loses brand context. Both is the answer — front pins identity, back pins composition, web_search bridges any gap.

**What changed in §5 Step 3:**
1. **Input shape:** was `{ photo: <bytes> }`, becomes `{ photos: { front: <bytes>, back: <bytes> } }`. Both required when `STRAND_AI_PROVIDER_PRODUCT_PHOTO=claude`. Lovable+Gemini path stays single-photo for back-compat.
2. **Vision content blocks:** two image blocks in the user message. Front first (brand + product name + marketing claims), back second (ingredient panel + usage + regulatory). Each labelled in an adjacent text block so Claude can attribute facts correctly.
3. **Task instructions update:** *"You're looking at two photos of the same product — front (brand + product name) and back (ingredient panel + usage instructions). Read both. If either is missing, blurry, or in the wrong language, use web_search to fill the gap. The full canonical product details should be reflected in your `return_product_analysis` output: `ingredients[]` is the COMPLETE INCI list, `product_name` and `brand` match what the brand actually calls it, `category` and `use_cases` align with the brand's positioning."*
4. **Token cost (revised again):** ~3000 tokens for two images + ~2.2K text input + ~800 output + 2–3 web_search invocations → **~$0.04–0.06 per call** (was ~$0.03–0.05 single-photo+search; was ~$0.014 vision-only). Updated in §5 Step 3 and the cost-projection note.
5. **Verification step (§8 Step 11b) updated:** photograph FRONT + BACK of a product with partial label info on the front AND partial INCI on the back. Verify `ingredients[]` returns the FULL INCI, `product_name` is the brand's actual SKU name (not a descriptor), all `return_product_analysis` fields populated as if the user had pasted a URL. Also confirm the strict-validation path — submit with one photo missing and the server returns HTTP 400 with the exact message *"STRAND needs both the front and back of the product to give you a full analysis."* with no silent fallback to single-photo.
6. **UX flow (guided two-step):** client opens camera with "Photograph the front of the product"; on capture auto-advances to "Now photograph the back where the ingredients are listed"; both required to submit; no "I only have one" escape. Single-photo input is unsupported when the flag is `claude`.

**What did NOT change:**
- **Schema:** `return_product_analysis` is identical. Front/back live in input only; output payload shape is unchanged.
- **Tools:** `web_search` still bounded at `max_uses: 4`. RAG still off for this function.
- **Persona footer rule:** book-derived guidance gets the formal "Read more — …" line; web-derived facts referenced inline naturally. Unchanged.
- **Rollout flag:** `STRAND_AI_PROVIDER_PRODUCT_PHOTO=claude`, per-function only.
- **Step 4a, 4b, 5a, 5b** structure, schemas, and flags untouched.

**Sequencing note attached to this revision:**
- The Step 3 work splits into two PRs to keep server and client landings reviewable independently:
  1. **Server PR** — strict dual-photo validation, dual-image Anthropic call, schema-shared tool. Lands first. While this PR is live but the client capture UX hasn't shipped, the existing single-photo client path is broken under `STRAND_AI_PROVIDER_PRODUCT_PHOTO=claude` — leave the flag on `lovable` between the two PRs.
