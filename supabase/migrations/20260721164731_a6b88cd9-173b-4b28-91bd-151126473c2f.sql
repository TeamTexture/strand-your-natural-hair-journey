CREATE OR REPLACE FUNCTION public.brand_catalogue_items(_kind text DEFAULT 'all'::text, _search text DEFAULT NULL::text, _limit integer DEFAULT 80)
RETURNS TABLE(kind text, source_id uuid, name text, brand text, category text, image_url text, ingredients text[], tool_kind text, key_features text[], materials text[], source_url text, user_count bigint)
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
BEGIN
  ...
  WITH product_rows AS (
    SELECT
      ...
      MIN(up.name)::text AS name,
      NULLIF(MIN(NULLIF(up.brand, '')), '')::text AS brand,
      NULLIF(MIN(NULLIF(up.category, '')), '')::text AS category,
      NULLIF(MAX(NULLIF(up.image_url, '')), '')::text AS image_url,
      COALESCE((
        ...
        FROM (
          ...
          WHERE lower(trim(up2.name)) = lower(trim(MIN(up.name)))
            AND lower(trim(COALESCE(up2.brand, ''))) = lower(trim(COALESCE(MIN(up.brand), '')))
          LIMIT 80
        ) i
        WHERE NULLIF(trim(ing), '') IS NOT NULL
      ), '{}'::text[])::text[] AS ingredients,
    ...
    GROUP BY lower(trim(up.name)), lower(trim(COALESCE(up.brand, '')))
  ),
  tool_rows AS (
    SELECT
      'tool'::text AS kind,
      MIN(ut.id)::uuid AS source_id,
      MIN(ut.name)::text AS name,
      NULLIF(MIN(NULLIF(ut.brand, '')), '')::text AS brand,
      NULLIF(MIN(NULLIF(ut.category, '')), '')::text AS category,
      NULLIF(MAX(NULLIF(ut.image_url, '')), '')::text AS image_url,
      '{}'::text[] AS ingredients,
      NULLIF(MIN(NULLIF(ut.category, '')), '')::text AS tool_kind,
      COALESCE(ARRAY_REMOVE(ARRAY_AGG(DISTINCT NULLIF(ut.category, '')), NULL), '{}'::text[])::text[] AS key_features,
      '{}'::text[] AS materials,
      NULLIF(MAX(NULLIF(ut.source_url, '')), '')::text AS source_url,
    ...
    GROUP BY lower(trim(ut.name)), lower(trim(COALESCE(ut.brand, '')))
  ),
  combined AS (
...
REVOKE ALL ON FUNCTION public.brand_catalogue_items(text, text, integer) FROM anon;
