...
ALTER TABLE public.brand_offers ALTER COLUMN headline DROP NOT NULL;
...
SET headline = NULL,
    updated_at = now()
FROM (
  SELECT DISTINCT ON (offer_id) offer_id, headline
  FROM public.brand_offer_revisions
  WHERE status = 'approved'
  ORDER BY offer_id, reviewed_at DESC NULLS LAST
) latest
WHERE latest.offer_id = o.id
  AND (latest.headline IS NULL OR latest.headline = '')
  AND o.headline IS NOT NULL;
