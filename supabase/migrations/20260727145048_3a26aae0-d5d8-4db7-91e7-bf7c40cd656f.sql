...
BEGIN
  ...
  -- Passport access is only granted with explicit member consent (GDPR).
  IF COALESCE(_share_passport_consent, false) THEN
...
REVOKE ALL ON FUNCTION public.send_enquiry_with_access(uuid,text,text,text,text,text,text,text,boolean) FROM public;
GRANT EXECUTE ON FUNCTION public.send_enquiry_with_access(uuid,text,text,text,text,text,text,text,boolean) TO authenticated;

DROP FUNCTION IF EXISTS public.send_enquiry_with_access(uuid,text,text,text,text,text,text,text);