CREATE OR REPLACE FUNCTION public.has_professional_undertaking(_pro uuid)
RETURNS boolean
LANGUAGE sql
...
AS $function$
  SELECT coalesce((
    ...
    WHERE c.user_id = _pro
      AND c.consent_key = 'professional_data_handling'
    ORDER BY c.granted_at DESC
    LIMIT 1
  ), false)
$function$;

REVOKE ALL ON FUNCTION public.has_professional_undertaking(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.has_professional_undertaking(uuid) TO authenticated, service_role;

-- Single choke point for every passport / health-record RLS policy.
...
AS $function$
  ...
  AND (
    ...
    OR public.has_professional_undertaking(_pro)
  )
$function$;

-- The undertaking no longer gates initial sign-in for professionals.
...
BEGIN
  ...
  -- professional_data_handling is deliberately NOT here: it is asked for on
  -- entering the professional view and gates client passport access only.
