...
CREATE OR REPLACE FUNCTION public.forum_author_meta(_user_ids uuid[])
 RETURNS TABLE(user_id uuid, display_name text, avatar_url text, city text, goal_title text, hair_type text)
 LANGUAGE plpgsql
...
BEGIN
  ...
  SELECT
    ...
    p.display_name,
    p.avatar_url,
    -- Map UK outward postcode area (letters only) to a city name.
    (
      WITH area AS (
