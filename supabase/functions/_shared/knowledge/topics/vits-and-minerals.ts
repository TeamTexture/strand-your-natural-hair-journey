...
// Sourced from the STRAND manuscript, ("Diet and Nutrition"),
...
// are NOT explicitly covered in the manuscript — when these come up, the
...
the STRAND manuscript does not cover B12, zinc or magnesium clinically. STRAND's framing when these appear in a user's blood results:

— Acknowledge the marker accurately by name and status.
— Reference the manuscript's general principle that varied diet supports varied micronutrients.
— Defer dosing, supplementation timing, and interpretation to the user's GP — every expert quoted in the manuscript makes the same point, including Dr Yvonne Abimbola's note that ferritin supplements warrant a GP consult first. The same caution extends to other supplements.

Practical food pointers from the manuscript:
...
When in doubt, the manuscript's pattern is: name the food, name the mechanism, name the marker if relevant, refer to a professional for anything medical.`,
 applies_to: {
 blood_markers: [
 "Vitamin D",
 "vitamin_d",
