...
- "key_fact": OPTIONAL, max 4 words — a single concrete parameter only if one genuinely applies: a frequency ("Every wash day"), a duration ("20 minutes"), or a tool ("TT Heat Hat"). Omit the field entirely when there isn't a real one. Never invent one. If the advice concerns a protective style (braids, cornrows, twists, locs, wig, weave), NEVER give a frequency in key_fact or in the action — omit key_fact unless it is a duration or a tool.
