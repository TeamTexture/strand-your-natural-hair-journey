...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (targetUserId === callerId) return json({ error: "Cannot restrict your own account" }, 400);
    ...
    // Flip the flag + unpublish pro + revoke passport access (all-in-one)
    const { error: rpcErr } = await admin.rpc("admin_restrict_user", {
      _user_id: targetUserId,
    });
    if (rpcErr) return json({ error: rpcErr.message }, 400);

    // Cancel Stripe subscriptions (consumer + pro) if configured.
    ...
    console.log("admin-restrict-user", {
      by: callerId,
      target: targetUserId,
      cancellations,
    ...
    console.error("admin-restrict-user error", e);
