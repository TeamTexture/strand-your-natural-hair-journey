...
2) next_wash_tip: an object with { action, why }. BE SUCCINCT — this is a home-screen card, not a paragraph.
   - action: SHORT CARD HEADER only — 3 to 7 words, Title Case, no trailing period. HOLISTIC: captures the WHOLE arc of the tip, not one isolated step. Pair linked moves with a comma or "and" ("Cleanse Then Deep-Condition Under Heat", "Repeat Sequence, Add Midweek Moisture", "Protect Ends, Lead With Moisture"). Not a full sentence.
   - ABSOLUTE HEADER FORBIDS: do NOT write the header as a single-step instruction that names one product ("Cleanse With The [Brand] Shampoo", "Use The [Brand] Mask", "Apply [Product]"). Do NOT name a specific product in the header at all — product names belong in the body. If the body prescribes cleanse + condition, cleanse + mask, cleanse + heat, cleanse + midweek moisture, or any multi-step arc, the header MUST reference each part in generic terms ("Double Cleanse Then Deep-Condition", "Cleanse, Condition & Seal Moisture"). Only when the body genuinely prescribes ONE move (e.g. only "protect your ends") may the header name that single move.
   ...
   - HEADER↔BODY ALIGNMENT (STRICT + HOLISTIC): the \`action\` header MUST cover EVERY move \`why\` recommends. Draft \`why\` first, then extract the header from it.
     • Enumerate the moves in \`why\` (cleanse, condition, mask, heat, leave-in, midweek moisture, protect ends, style, etc.). The header must reference each one — never just the first step, never just the product.
     • Before returning: reread. Could a reader summarise the body from just the header alone, without missing a step? If not, rewrite so the header covers the whole arc.
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    return json(200, await sanitiseAndLog(result, "wash-day-observation", { context: body.context, grounding: grounding.sourceText, chapters: grounding.chapters }));
  } catch (e) {
