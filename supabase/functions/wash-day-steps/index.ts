...
/** Step count budget per support level. */
...
/** Enforce the caps, ordering and step budget on the model's output. */
export function normaliseSteps(raw: unknown, level: number): WashStep[] {
  ...
  for (const item of raw) {
    ...
    // ONE IDEA, ONCE — drop a step whose headline repeats an earlier one.
